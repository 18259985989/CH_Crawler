...
class Abnormal(Base):
    ...
    baseInfo = relationship("BaseInfo", backref=backref('abnormalRel', order_by=id, cascade="all, delete-orphan", passive_deletes=True))

    def __repr__(self):
        return "id: {0}, c_id: {1}, put_date: {2}, put_reason: {3}, put_department: {4}, remove_date: {5}, remove_reason: {6}, remove_department: {7}".format(self.id,
                                                                                                                                                              self.c_id,
                                                                                                                                                              self.put_date,
                                                                                                                                                              self.put_reason,
                                                                                                                                                              self.put_department,
...
class BidInfo(Base):
    ...
    baseInfo = relationship("BaseInfo", backref=backref('bidInfoRel', order_by=id, cascade="all, delete-orphan", passive_deletes=True))

    def __repr__(self):
        return "id: {0}, c_id: {1}, title: {2}, publish_date: {3}, purchaser: {4}, proxy: {5}, content: {6}".format(self.id, self.c_id, self.title, self.publish_date,
...
class Branch(Base):
    ...
    baseInfo = relationship("BaseInfo", backref=backref('branchRel', order_by=id, cascade="all, delete-orphan", passive_deletes=True))
    ...
    def __repr__(self):
        return "id: {0}, c_id: {1}, branch_name: {2}, credit_code: {3}, Enterprise_gszch: {4}, reg_authority: {5}, legal_person: {6}, industry: {7}, estiblish_date: {8}, business_state: {9}".format(
            self.id, self.c_id, self.branch_name, self.credit_code, self.Enterprise_gszch, self.reg_authority, self.legal_person, self.industry, self.estiblish_date,
            self.business_state)
...
class ChangeInfo(Base):
    ...
    baseInfo = relationship("BaseInfo", backref=backref('changeInfoRel', order_by=id, cascade="all, delete-orphan", passive_deletes=True))

    def __repr__(self):
        return "id: {0}, c_id: {1}, change_item: {2}, before_change: {3}, after_change: {4}, change_date: {5}".format(self.id, self.c_id, self.change_item,
...
class CheckInfo(Base):
    ...
    baseInfo = relationship("BaseInfo", backref=backref('checkInfoRel', order_by=id, cascade="all, delete-orphan", passive_deletes=True))

    def __repr__(self):
        return "id: {0}, c_id: {1}, check_authority: {2}, check_date: {3}, check_result: {4}, check_type: {5}".format(self.id, self.c_id, self.check_authority,
...
class CompanyMortgage(Base):
    ...
    baseInfo = relationship("BaseInfo", backref=backref('companyMortgageRel', order_by=id, cascade="all, delete-orphan", passive_deletes=True))
    ...
    def __repr__(self):
        return "id: {0}, c_id: {1}, mortgage_name: {2}, licence_type: {3}, identify_number: {4}, mortgage_type: {5}, address: {6}, reg_number: {7}, reg_date: {8}, reg_authority: {9}, guaranteed_amount: {10}, public_date: {11}, currency: {12}, cancel_date: {13}, cancel_authority: {14}, cancel_reason: {15}, warrant_type: {16}, term: {17}, scope: {18}, status: {19}, note: {20}".format(
            self.id, self.c_id, self.mortgage_name, self.licence_type, self.identify_number, self.mortgage_type, self.address, self.reg_number, self.reg_date,
            self.reg_authority, self.guaranteed_amount, self.public_date, self.currency, self.cancel_date, self.cancel_authority, self.cancel_reason, self.warrant_type,
...
class CompanyMortgageChange(Base):
    ...
    companyMortgage = relationship("CompanyMortgage",
                                   backref=backref('companyMortgageChangeRel', order_by=id, cascade="all, delete-orphan", passive_deletes=True))

    def __repr__(self):
        return "id: {0}, reg_id: {1}, change_date: {2}, change_content: {3}".format(self.id, self.reg_id, self.change_date, self.change_content)
...
class CompanyMortgageCollateral(Base):
    ...
    companyMortgage = relationship("CompanyMortgage",
                                   backref=backref('companyMortgageCollateralRel', order_by=id, cascade="all, delete-orphan", passive_deletes=True))

    def __repr__(self):
        return "id: {0}, reg_id: {1}, collateral: {2}, belong: {3}, summary: {4}".format(self.id, self.reg_id, self.collateral, self.belong, self.summary)
...
class CompanyMortgagePledgee(Base):
    ...
    companyMortgage = relationship("CompanyMortgage",
                                   backref=backref('companyMortgagePledgeeRel', order_by=id, cascade="all, delete-orphan", passive_deletes=True))

    def __repr__(self):
        return "id: {0}, reg_id: {1}, people_name: {2}, licence_type: {3}, identify_number: {4}, address: {5}".format(self.id, self.reg_id, self.people_name,
...
class CopyrightSoftware(Base):
    ...
    baseInfo = relationship("BaseInfo", backref=backref('copyrightSoftwareRel', order_by=id, cascade="all, delete-orphan", passive_deletes=True))
    ...
    def __repr__(self):
        return "id: {0}, c_id: {1}, software_name: {2}, reg_number: {3}, work_type: {4}, finish_date: {5}, publish_date: {6}, reg_date: {7}, simple_name: {8}, cat_number: {9}, version: {10}, author: {11}".format(
            self.id, self.c_id, self.software_name, self.reg_number, self.work_type, self.finish_date, self.publish_date, self.reg_date, self.simple_name,
...
class CopyrightWork(Base):
    ...
    baseInfo = relationship("BaseInfo", backref=backref('copyrightWorkRel', order_by=id, cascade="all, delete-orphan", passive_deletes=True))

    def __repr__(self):
        return "id: {0}, c_id: {1}, work_name: {2}, reg_number: {3}, work_type: {4}, finish_date: {5}, publish_date: {6}, reg_date: {7}, author: {8}".format(self.id,
                                                                                                                                                             self.c_id,
                                                                                                                                                             self.work_name,
                                                                                                                                                             self.reg_number,
                                                                                                                                                             self.work_type,
...
class CourtAuto(Base):
    ...
    baseInfo = relationship("BaseInfo", backref=backref('courtAutoRel', order_by=id, cascade="all, delete-orphan", passive_deletes=True))
    ...
    def __repr__(self):
        return "id: {0}, c_id: {1}, start_date: {2}, case_reason: {3}, case_number: {4}, litigant: {5}, plaintiff: {6}, defendant: {7}, court: {8}, courtroom: {9}".format(
            self.id, self.c_id, self.start_date, self.case_reason, self.case_number, self.litigant, self.plaintiff, self.defendant, self.court, self.courtroom)
...
class CourtNotice(Base):
    ...
    baseInfo = relationship("BaseInfo", backref=backref('courtNoticeRel', order_by=id, cascade="all, delete-orphan", passive_deletes=True))
...
class DishonestInfo(Base):
    ...
    baseInfo = relationship("BaseInfo", backref=backref('dishonestInfoRel', order_by=id, cascade="all, delete-orphan", passive_deletes=True))
    ...
    def __repr__(self):
        return "id: {0}, c_id: {1}, ent_name: {2}, card_number: {3}, area: {4}, reg_date: {5}, publish_date: {6}, gist_unit: {7}, court: {8}, gist_num: {9}, case_code: {10}, duty: {11}, performance: {12}, disrupttype_name: {13}".format(
            self.id, self.c_id, self.ent_name, self.card_number, self.area, self.reg_date, self.publish_date, self.gist_unit, self.court, self.gist_num, self.case_code,
...
class EquityChange(Base):
    ...
    baseInfo = relationship("BaseInfo", backref=backref('equityChangeRel', order_by=id, cascade="all, delete-orphan", passive_deletes=True))

    def __repr__(self):
        return "id: {0}, c_id: {1}, shareholder: {2}, alt_date: {3}, trans_before: {4}, trans_after: {5}, publish_date: {6}".format(self.id, self.c_id, self.shareholder,
...
class EquityPledge(Base):
    ...
    baseInfo = relationship("BaseInfo", backref=backref('equityPledgeRel', order_by=id, cascade="all, delete-orphan", passive_deletes=True))
    ...
    def __repr__(self):
        return "id: {0}, c_id: {1}, reg_number: {2}, reg_date: {3}, pledgor: {4}, pledge_equity: {5}, pledgee: {6}, pledgee_type: {7}, state: {8}, currency: {9}, cancel_date: {10}, cancel_reason: {11}, certif_number_gor: {12}, certif_number_gee: {13}".format(
            self.id, self.c_id, self.reg_number, self.reg_date, self.pledgor, self.pledge_equity, self.pledgee, self.pledgee_type, self.state, self.currency,
...
class IllegalInfo(Base):
    ...
    baseInfo = relationship("BaseInfo", backref=backref('illegalInfoRel', order_by=id, cascade="all, delete-orphan", passive_deletes=True))
...
class ImExportBase(Base):
    ...
    baseInfo = relationship("BaseInfo", backref=backref('imExportBaseRel', order_by=id, cascade="all, delete-orphan", passive_deletes=True))
    ...
    def __repr__(self):
        return "id: {0}, c_id: {1}, crcode: {2}, record_date: {3}, customs_registered_address: {4}, administrative_division: {5}, economic_division: {6}, management_category: {7}, special_trade_area: {8}, industry_category: {9}, validity_date: {10}, status: {11}, annual_report: {12}, types: {13}".format(
            self.id, self.c_id, self.crcode, self.record_date, self.customs_registered_address, self.administrative_division, self.economic_division,
            self.management_category, self.special_trade_area, self.industry_category, self.validity_date, self.status, self.annual_report, self.types)
...
class ImExportCredit(Base):
    ...
    baseInfo = relationship("BaseInfo", backref=backref('imExportCreditRel', order_by=id, cascade="all, delete-orphan", passive_deletes=True))

    def __repr__(self):
        return "id: {0}, c_id: {1}, credit_rating: {2}, authentication_code: {3}, identification_time: {4}".format(self.id, self.c_id, self.credit_rating,
...
class ImExportPunish(Base):
    ...
    baseInfo = relationship("BaseInfo", backref=backref('imExportPunishRel', order_by=id, cascade="all, delete-orphan", passive_deletes=True))

    def __repr__(self):
        return "id: {0}, c_id: {1}, decision_number: {2}, penalty_date: {3}, party: {4}, nature_of_case: {5}".format(self.id, self.c_id, self.decision_number,
...
class Invest(Base):
    ...
    baseInfo = relationship("BaseInfo", backref=backref('investRel', order_by=id, cascade="all, delete-orphan", passive_deletes=True))
    ...
    def __repr__(self):
        return "id: {0}, c_id: {1}, ent_name: {2}, credit_code: {3}, invest_amount: {4}, invest_percent: {5}, ent_type: {6}, business_scope: {7}, business_state: {8}, alias: {9}, estiblish_date: {10}, legal_person: {11}, industry: {12}, reg_capital: {13}, state: {14}".format(
            self.id, self.c_id, self.ent_name, self.credit_code, self.invest_amount, self.invest_percent, self.ent_type, self.business_scope, self.business_state,
...
class JudicialAssistance(Base):
    ...
    baseInfo = relationship("BaseInfo", backref=backref('judicialAssistanceRel', order_by=id, cascade="all, delete-orphan", passive_deletes=True))
    ...
    def __repr__(self):
        return "id: {0}, c_id: {1}, public_date: {2}, executed: {3}, equity_amount: {4}, equity_state: {5}, execute_notice: {6}, execute_court: {7}, execute_list: {8}, execute_order: {9}, license_type: {10}, license_number: {11}, from_date: {12}, to_date: {13}, period: {14}".format(
            self.id, self.c_id, self.public_date, self.executed, self.equity_amount, self.equity_state, self.execute_notice, self.execute_court, self.execute_list,
...
class JudicialSale(Base):
    ...
    baseInfo = relationship("BaseInfo", backref=backref('judicialSaleRel', order_by=id, cascade="all, delete-orphan", passive_deletes=True))

    def __repr__(self):
        return "id: {0}, c_id: {1}, title: {2}, title_money: {3}, court: {4}, public_date: {5}, initial_price: {6}, consult_price: {7}".format(self.id, self.c_id,
                                                                                                                                               self.title,
                                                                                                                                               self.title_money,
                                                                                                                                               self.court,
                                                                                                                                               self.public_date,
...
class LawsuitBasic(Base):
    ...
    baseInfo = relationship("BaseInfo", backref=backref('lawsuitBasicRel', order_by=id, cascade="all, delete-orphan", passive_deletes=True))
    ...
    def __repr__(self):
        return "id: {0}, c_id: {1}, title: {2}, case_type: {3}, submittime: {4}, casereason: {5}, plaintiff: {6}, defendants: {7}, caseno: {8}, lawsuit_detail: {9}".format(
            self.id, self.c_id, self.title, self.case_type, self.submittime, self.casereason, self.plaintiff, self.defendants, self.caseno, self.lawsuit_detail)
...
class LicenceInfo(Base):
    ...
    baseInfo = relationship("BaseInfo", backref=backref('licenceInfoRel', order_by=id, cascade="all, delete-orphan", passive_deletes=True))
    ...
    def __repr__(self):
        return "id: {0}, c_id: {1}, licence_number: {2}, licence_name: {3}, licence_anth: {4}, from_date: {5}, to_date: {6}, licence_content: {7}, legal_person: {8}, audit_type: {9}".format(
            self.id, self.c_id, self.licence_number, self.licence_name, self.licence_anth, self.from_date, self.to_date, self.licence_content, self.legal_person,
            self.audit_type)
...
class Members(Base):
    ...
    baseInfo = relationship("BaseInfo", backref=backref('membersRel', order_by=id, cascade="all, delete-orphan", passive_deletes=True))

    def __repr__(self):
        return "id: {0}, c_id: {1}, person_name: {2}, position: {3}".format(self.id, self.c_id, self.person_name, self.position)
...
class OwingTax(Base):
    ...
    baseInfo = relationship("BaseInfo", backref=backref('owingTaxRel', order_by=id, cascade="all, delete-orphan", passive_deletes=True))
    ...
    def __repr__(self):
        return "id: {0}, c_id: {1}, tax_id_number: {2}, publish_date: {3}, tax_category: {4}, own_tax_balance: {5}, new_own_tax_balance: {6}, department: {7}".format(
            self.id, self.c_id, self.tax_id_number, self.publish_date, self.tax_category, self.own_tax_balance, self.new_own_tax_balance, self.department)
...
class Patent(Base):
    ...
    baseInfo = relationship("BaseInfo", backref=backref('patentRel', order_by=id, cascade="all, delete-orphan", passive_deletes=True))
    ...
    def __repr__(self):
        return "id: {0}, c_id: {1}, patent_name: {2}, apply_number: {3}, pub_number: {4}, cat_number: {5}, apply_date: {6}, app_publish_date: {7}, applicant_name: {8}, inventor: {9}, agent: {10}, agency: {11}, address: {12}, abstracts: {13}, img_url: {14}".format(
            self.id, self.c_id, self.patent_name, self.apply_number, self.pub_number, self.cat_number, self.apply_date, self.app_publish_date, self.applicant_name,
...
class Pledgereg(Base):
    ...
    baseInfo = relationship("BaseInfo", backref=backref('pledgeregRel', order_by=id, cascade="all, delete-orphan", passive_deletes=True))
    ...
    def __repr__(self):
        return "id: {0}, c_id: {1}, reg_number: {2}, ent_name: {3}, kinds: {4}, pledgor_name: {5}, pledge_from: {6}, pledge_to: {7}, public_date: {8}, tm_name: {9}, state: {10}, pledgee_name: {11}".format(
            self.id, self.c_id, self.reg_number, self.ent_name, self.kinds, self.pledgor_name, self.pledge_from, self.pledge_to, self.public_date, self.tm_name,
...
class Product(Base):
    ...
    baseInfo = relationship("BaseInfo", backref=backref('productRel', order_by=id, cascade="all, delete-orphan", passive_deletes=True))

    def __repr__(self):
        return "id: {0}, c_id: {1}, app_name: {2}, filter_name: {3}, classes: {4}, brief: {5}, icon: {6}, pro_type: {7}".format(self.id, self.c_id, self.app_name,
...
class Punishment(Base):
    ...
    baseInfo = relationship("BaseInfo", backref=backref('punishmentRel', order_by=id, cascade="all, delete-orphan", passive_deletes=True))
    ...
    def __repr__(self):
        return "id: {0}, c_id: {1}, penish_dec_num: {2}, illeg_type: {3}, punish_content: {4}, punish_auth: {5}, decision_date: {6}, punish_name: {7}, punish_type: {8}, punish_type_second: {9}, evidence: {10}".format(
            self.id, self.c_id, self.penish_dec_num, self.illeg_type, self.punish_content, self.punish_auth, self.decision_date, self.punish_name, self.punish_type,
...
class Qualification(Base):
    ...
    baseInfo = relationship("BaseInfo", backref=backref('qualificationRel', order_by=id, cascade="all, delete-orphan", passive_deletes=True))

    def __repr__(self):
        return "id: {0}, c_id: {1}, certificate_name: {2}, cert_number: {3}, start_date: {4}, end_date: {5}".format(self.id, self.c_id, self.certificate_name,
...
class Shareholder(Base):
    ...
    baseInfo = relationship("BaseInfo", backref=backref('shareholderRel', order_by=id, cascade="all, delete-orphan", passive_deletes=True))
    ...
    def __repr__(self):
        return "id: {0}, shareholder: {1}, shareholder_type: {2}, blic_type: {3}, blic_number: {4}, ent_type: {5}, con_capital: {6}, con_capital_date: {7}, con_paid_in_capital: {8}, paid_in_capital_date: {9}, share_ratio: {10}, currency: {11}, c_id: {12}".format(
            self.id, self.shareholder, self.shareholder_type, self.blic_type, self.blic_number, self.ent_type, self.con_capital, self.con_capital_date,
...
class Taxcred(Base):
    ...
    baseInfo = relationship("BaseInfo", backref=backref('taxcredRel', order_by=id, cascade="all, delete-orphan", passive_deletes=True))

    def __repr__(self):
        return "id: {0}, c_id: {1}, id_number: {2}, years: {3}, eval_department: {4}, grade: {5}, eval_type: {6}".format(self.id, self.c_id, self.id_number, self.years,
...
class TmInfo(Base):
    ...
    baseInfo = relationship("BaseInfo", backref=backref('tmInfoRel', order_by=id, cascade="all, delete-orphan", passive_deletes=True))
    ...
    def __repr__(self):
        return "id: {0}, c_id: {1}, tm_name: {2}, reg_number: {3}, tm_type: {4}, reg_annc_date: {5}, reg_annc_issue: {6}, property_bgn_date: {7}, property_end_date: {8}, goods_name: {9}, apply_date: {10}, state: {11}, applicant: {12}, apply_process: {13}, pic_url: {14}".format(
            self.id, self.c_id, self.tm_name, self.reg_number, self.tm_type, self.reg_annc_date, self.reg_annc_issue, self.property_bgn_date, self.property_end_date,
...
class ToExecute(Base):
    ...
    baseInfo = relationship("BaseInfo", backref=backref('toExecuteRel', order_by=id, cascade="all, delete-orphan", passive_deletes=True))
...
class WebsiteInfo(Base):
    ...
    baseInfo = relationship("BaseInfo", backref=backref('websiteInfoRel', order_by=id, cascade="all, delete-orphan", passive_deletes=True))

    def __repr__(self):
        return "id: {0}, c_id: {1}, web_name: {2}, web_site: {3}, examine_date: {4}, liscense: {5}, company_type: {6}, ym: {7}".format(self.id, self.c_id, self.web_name,
...
class YearReport(Base):
    ...
    baseInfo = relationship("BaseInfo", backref=backref('yearReportRel', order_by=id, cascade="all, delete-orphan", passive_deletes=True))
    ...
    def __repr__(self):
        return "id: {0}, c_id: {1}, company_name: {2}, report_year: {3}, reg_number: {4}, manage_state: {5}, employee_num: {6}, phone_number: {7}, email: {8}, postal_address: {9}, postcode: {10}".format(
            self.id, self.c_id, self.company_name, self.report_year, self.reg_number, self.manage_state, self.employee_num, self.phone_number, self.email,
...
class YearReportAssert(Base):
    ...
    baseInfo = relationship("BaseInfo", backref=backref('yearReportAssertRel', order_by=id, cascade="all, delete-orphan", passive_deletes=True))
    ...
    def __repr__(self):
        return "id: {0}, c_id: {1}, total_assets: {2}, total_equity: {3}, total_sales: {4}, total_profit: {5}, prime_busprofit: {6}, retained_profit: {7}, total_tax: {8}, total_liability: {9}, report_year: {10}".format(
            self.id, self.c_id, self.total_assets, self.total_equity, self.total_sales, self.total_profit, self.prime_busprofit, self.retained_profit, self.total_tax,
...
class YearReportSocialSecurity(Base):
    ...
    baseInfo = relationship("BaseInfo", backref=backref('yearReportSocialSecurityRel', order_by=id, cascade="all, delete-orphan", passive_deletes=True))
    ...
    def __repr__(self):
        return "id: {0}, c_id: {1}, endowment_insurance: {2}, medical_insurance: {3}, maternit_insurance: {4}, unemployment_insurance: {5}, employmentinjury_insurance: {6}, endowment_insurance_base: {7}, medical_insurance_base: {8}, maternit_insurance_base: {9}, unemployment_insurance_base: {10}, employmentinjury_insurance_base: {11}, endowment_insurance_payamount: {12}, medical_insurance_payamount: {13}, maternit_insurance_payamount: {14}, unemployment_insurance_payamount: {15}, employmentinjury_insurance_payamount: {16}, endowment_insurance_oweamount: {17}, medical_insurance_oweamount: {18}, maternit_insurance_oweamount: {19}, unemployment_insurance_oweamount: {20}, employmentinjury_insurance_oweamount: {21}, report_year: {22}".format(
            self.id, self.c_id, self.endowment_insurance, self.medical_insurance, self.maternit_insurance, self.unemployment_insurance, self.employmentinjury_insurance,
            self.endowment_insurance_base, self.medical_insurance_base, self.maternit_insurance_base, self.unemployment_insurance_base,
            self.employmentinjury_insurance_base, self.endowment_insurance_payamount, self.medical_insurance_payamount, self.maternit_insurance_payamount,
            self.unemployment_insurance_payamount, self.employmentinjury_insurance_payamount, self.endowment_insurance_oweamount, self.medical_insurance_oweamount,
            self.maternit_insurance_oweamount, self.unemployment_insurance_oweamount, self.employmentinjury_insurance_oweamount, self.report_year)
...
class YearReportStore(Base):
    ...
    baseInfo = relationship("BaseInfo", backref=backref('yearReportStoreRel', order_by=id, cascade="all, delete-orphan", passive_deletes=True))

    def __repr__(self):
        return "id: {0}, c_id: {1}, web_name: {2}, web_type: {3}, web_site: {4}, report_year: {5}".format(self.id, self.c_id, self.web_name, self.web_type, self.web_site,
                                                                                                          self.report_year)
