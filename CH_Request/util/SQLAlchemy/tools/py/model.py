...
class Abnormal(Base):
    ...
    def __repr__(self):
        return "id: {0}, c_id: {1}, put_date: {2}, put_reason: {3}, put_department: {4}, remove_date: {5}, remove_reason: {6}, remove_department: {7}, KEY: {8}".format(
            self.id, self.c_id, self.put_date, self.put_reason, self.put_department, self.remove_date,
            self.remove_reason, self.remove_department, self.KEY)
...
class BidInfo(Base):
    ...
    def __repr__(self):
        return "id: {0}, c_id: {1}, title: {2}, publish_date: {3}, purchaser: {4}, proxy: {5}, content: {6}, KEY: {7}".format(
            self.id, self.c_id, self.title, self.publish_date, self.purchaser, self.proxy, self.content, self.KEY)
...
class Branch(Base):
    ...
    def __repr__(self):
        return "id: {0}, c_id: {1}, branch_name: {2}, credit_code: {3}, Enterprise_gszch: {4}, reg_authority: {5}, legal_person: {6}, industry: {7}, estiblish_date: {8}, business_state: {9}, KEY: {10}".format(
            self.id, self.c_id, self.branch_name, self.credit_code, self.Enterprise_gszch, self.reg_authority,
            self.legal_person, self.industry, self.estiblish_date, self.business_state, self.KEY)
...
class ChangeInfo(Base):
    ...
    def __repr__(self):
        return "id: {0}, c_id: {1}, change_item: {2}, before_change: {3}, after_change: {4}, change_date: {5}, KEY: {6}".format(
            self.id, self.c_id, self.change_item, self.before_change, self.after_change, self.change_date, self.KEY)
...
class CheckInfo(Base):
    ...
    def __repr__(self):
        return "id: {0}, c_id: {1}, check_authority: {2}, check_date: {3}, check_result: {4}, check_type: {5}, KEY: {6}".format(
            self.id, self.c_id, self.check_authority, self.check_date, self.check_result, self.check_type, self.KEY)
...
class CompanyMortgage(Base):
    ...
    def __repr__(self):
        return "id: {0}, c_id: {1}, mortgage_name: {2}, licence_type: {3}, identify_number: {4}, mortgage_type: {5}, address: {6}, reg_number: {7}, reg_date: {8}, reg_authority: {9}, guaranteed_amount: {10}, public_date: {11}, currency: {12}, cancel_date: {13}, cancel_authority: {14}, cancel_reason: {15}, warrant_type: {16}, term: {17}, scope: {18}, status: {19}, note: {20}, KEY: {21}".format(
            self.id, self.c_id, self.mortgage_name, self.licence_type, self.identify_number, self.mortgage_type,
            self.address, self.reg_number, self.reg_date, self.reg_authority, self.guaranteed_amount, self.public_date,
            self.currency, self.cancel_date, self.cancel_authority, self.cancel_reason, self.warrant_type, self.term,
            self.scope, self.status, self.note, self.KEY)
...
class CompanyMortgageChange(Base):
    ...
    def __repr__(self):
        return "id: {0}, reg_id: {1}, change_date: {2}, change_content: {3}, KEY: {4}".format(self.id, self.reg_id,
                                                                                              self.change_date,
                                                                                              self.change_content,
                                                                                              self.KEY)
...
class CompanyMortgageCollateral(Base):
    ...
    def __repr__(self):
        return "id: {0}, reg_id: {1}, collateral: {2}, belong: {3}, summary: {4}, KEY: {5}".format(self.id, self.reg_id,
                                                                                                   self.collateral,
                                                                                                   self.belong,
                                                                                                   self.summary,
                                                                                                   self.KEY)
...
class CompanyMortgagePledgee(Base):
    ...
    def __repr__(self):
        return "id: {0}, reg_id: {1}, people_name: {2}, licence_type: {3}, identify_number: {4}, address: {5}, KEY: {6}".format(
            self.id, self.reg_id, self.people_name, self.licence_type, self.identify_number, self.address, self.KEY)
...
class CopyrightSoftware(Base):
    ...
    def __repr__(self):
        return "id: {0}, c_id: {1}, software_name: {2}, reg_number: {3}, work_type: {4}, finish_date: {5}, publish_date: {6}, reg_date: {7}, simple_name: {8}, cat_number: {9}, version: {10}, author: {11}, KEY: {12}".format(
            self.id, self.c_id, self.software_name, self.reg_number, self.work_type, self.finish_date,
            self.publish_date, self.reg_date, self.simple_name, self.cat_number, self.version, self.author, self.KEY)
...
class CopyrightWork(Base):
    ...
    def __repr__(self):
        return "id: {0}, c_id: {1}, work_name: {2}, reg_number: {3}, work_type: {4}, finish_date: {5}, publish_date: {6}, reg_date: {7}, author: {8}, KEY: {9}".format(
            self.id, self.c_id, self.work_name, self.reg_number, self.work_type, self.finish_date, self.publish_date,
            self.reg_date, self.author, self.KEY)
...
class CourtAuto(Base):
    ...
    def __repr__(self):
        return "id: {0}, c_id: {1}, start_date: {2}, case_reason: {3}, case_number: {4}, litigant: {5}, plaintiff: {6}, defendant: {7}, court: {8}, courtroom: {9}, KEY: {10}".format(
            self.id, self.c_id, self.start_date, self.case_reason, self.case_number, self.litigant, self.plaintiff,
            self.defendant, self.court, self.courtroom, self.KEY)
...
class CourtNotice(Base):
    ...
    def __repr__(self):
        return "id: {0}, c_id: {1}, litigant: {2}, announce_type: {3}, publish_date: {4}, publish_page: {5}, appellant: {6}, court: {7}, content: {8}, province: {9}, announce_num: {10}, KEY: {11}".format(
            self.id, self.c_id, self.litigant, self.announce_type, self.publish_date, self.publish_page, self.appellant,
            self.court, self.content, self.province, self.announce_num, self.KEY)
...
class DishonestInfo(Base):
    ...
    def __repr__(self):
        return "id: {0}, c_id: {1}, ent_name: {2}, card_number: {3}, area: {4}, reg_date: {5}, publish_date: {6}, gist_unit: {7}, court: {8}, gist_num: {9}, case_code: {10}, duty: {11}, performance: {12}, disrupttype_name: {13}, KEY: {14}".format(
            self.id, self.c_id, self.ent_name, self.card_number, self.area, self.reg_date, self.publish_date,
            self.gist_unit, self.court, self.gist_num, self.case_code, self.duty, self.performance,
            self.disrupttype_name, self.KEY)
...
class EquityPledge(Base):
    ...
    def __repr__(self):
        return "id: {0}, c_id: {1}, reg_number: {2}, reg_date: {3}, pledgor: {4}, pledge_equity: {5}, pledgee: {6}, pledgee_type: {7}, state: {8}, currency: {9}, cancel_date: {10}, cancel_reason: {11}, certif_number_gor: {12}, certif_number_gee: {13}, KEY: {14}".format(
            self.id, self.c_id, self.reg_number, self.reg_date, self.pledgor, self.pledge_equity, self.pledgee,
            self.pledgee_type, self.state, self.currency, self.cancel_date, self.cancel_reason, self.certif_number_gor,
            self.certif_number_gee, self.KEY)
...
class ImExportBase(Base):
    ...
    def __repr__(self):
        return "id: {0}, c_id: {1}, crcode: {2}, record_date: {3}, customs_registered_address: {4}, administrative_division: {5}, economic_division: {6}, management_category: {7}, special_trade_area: {8}, industry_category: {9}, validity_date: {10}, status: {11}, annual_report: {12}, types: {13}, KEY: {14}".format(
            self.id, self.c_id, self.crcode, self.record_date, self.customs_registered_address,
            self.administrative_division, self.economic_division, self.management_category, self.special_trade_area,
            self.industry_category, self.validity_date, self.status, self.annual_report, self.types, self.KEY)
...
class Invest(Base):
    ...
    def __repr__(self):
        return "id: {0}, c_id: {1}, ent_name: {2}, credit_code: {3}, invest_amount: {4}, invest_percent: {5}, ent_type: {6}, business_scope: {7}, business_state: {8}, alias: {9}, estiblish_date: {10}, legal_person: {11}, industry: {12}, reg_capital: {13}, state: {14}, KEY: {15}".format(
            self.id, self.c_id, self.ent_name, self.credit_code, self.invest_amount, self.invest_percent, self.ent_type,
            self.business_scope, self.business_state, self.alias, self.estiblish_date, self.legal_person, self.industry,
            self.reg_capital, self.state, self.KEY)
...
class JudicialSale(Base):
    ...
    def __repr__(self):
        return "id: {0}, c_id: {1}, title: {2}, title_money: {3}, court: {4}, public_date: {5}, initial_price: {6}, consult_price: {7}, KEY: {8}".format(
            self.id, self.c_id, self.title, self.title_money, self.court, self.public_date, self.initial_price,
            self.consult_price, self.KEY)
...
class LawsuitBasic(Base):
    ...
    court = Column(String(60))
    decision_time = Column(DateTime)
    decide_reason = Column(Text)
    result = Column(Text)

    baseInfo = relationship("BaseInfo", backref=backref('lawsuitBasicRel', order_by=id, cascade="all, delete-orphan"))

    def __repr__(self):
        return "id: {0}, c_id: {1}, title: {2}, case_type: {3}, submittime: {4}, casereason: {5}, plaintiff: {6}, defendants: {7}, caseno: {8}, lawsuit_detail: {9}, court: {10}, decision_time: {11}, decide_reason: {12}, result: {13}, KEY: {14}".format(
            self.id, self.c_id, self.title, self.case_type, self.submittime, self.casereason, self.plaintiff,
            self.defendants, self.caseno, self.lawsuit_detail, self.court, self.decision_time, self.decide_reason,
...
class LicenceInfo(Base):
    ...
    def __repr__(self):
        return "id: {0}, c_id: {1}, licence_number: {2}, licence_name: {3}, licence_anth: {4}, from_date: {5}, to_date: {6}, licence_content: {7}, legal_person: {8}, audit_type: {9}, KEY: {10}".format(
            self.id, self.c_id, self.licence_number, self.licence_name, self.licence_anth, self.from_date, self.to_date,
            self.licence_content, self.legal_person, self.audit_type, self.KEY)
...
class Members(Base):
    ...
    def __repr__(self):
        return "id: {0}, c_id: {1}, person_name: {2}, position: {3}, KEY: {4}".format(self.id, self.c_id,
                                                                                      self.person_name, self.position,
                                                                                      self.KEY)
...
class OwingTax(Base):
    ...
    def __repr__(self):
        return "id: {0}, c_id: {1}, tax_id_number: {2}, publish_date: {3}, tax_category: {4}, own_tax_balance: {5}, new_own_tax_balance: {6}, department: {7}, KEY: {8}".format(
            self.id, self.c_id, self.tax_id_number, self.publish_date, self.tax_category, self.own_tax_balance,
            self.new_own_tax_balance, self.department, self.KEY)
...
class Patent(Base):
    ...
    def __repr__(self):
        return "id: {0}, c_id: {1}, patent_name: {2}, apply_number: {3}, pub_number: {4}, cat_number: {5}, apply_date: {6}, app_publish_date: {7}, applicant_name: {8}, inventor: {9}, agent: {10}, agency: {11}, address: {12}, abstracts: {13}, img_url: {14}, KEY: {15}".format(
            self.id, self.c_id, self.patent_name, self.apply_number, self.pub_number, self.cat_number, self.apply_date,
            self.app_publish_date, self.applicant_name, self.inventor, self.agent, self.agency, self.address,
            self.abstracts, self.img_url, self.KEY)
...
class Product(Base):
    ...
    def __repr__(self):
        return "id: {0}, c_id: {1}, app_name: {2}, filter_name: {3}, classes: {4}, brief: {5}, icon: {6}, pro_type: {7}, KEY: {8}".format(
            self.id, self.c_id, self.app_name, self.filter_name, self.classes, self.brief, self.icon, self.pro_type,
            self.KEY)
...
class Punishment(Base):
    ...
    def __repr__(self):
        return "id: {0}, c_id: {1}, penish_dec_num: {2}, illeg_type: {3}, punish_content: {4}, punish_auth: {5}, decision_date: {6}, punish_name: {7}, punish_type: {8}, punish_type_second: {9}, evidence: {10}, KEY: {11}".format(
            self.id, self.c_id, self.penish_dec_num, self.illeg_type, self.punish_content, self.punish_auth,
            self.decision_date, self.punish_name, self.punish_type, self.punish_type_second, self.evidence, self.KEY)
...
class Qualification(Base):
    ...
    def __repr__(self):
        return "id: {0}, c_id: {1}, certificate_name: {2}, cert_number: {3}, start_date: {4}, end_date: {5}, KEY: {6}".format(
            self.id, self.c_id, self.certificate_name, self.cert_number, self.start_date, self.end_date, self.KEY)
...
class Shareholder(Base):
    ...
    def __repr__(self):
        return "id: {0}, shareholder: {1}, shareholder_type: {2}, blic_type: {3}, blic_number: {4}, ent_type: {5}, con_capital: {6}, con_capital_date: {7}, con_paid_in_capital: {8}, paid_in_capital_date: {9}, share_ratio: {10}, currency: {11}, c_id: {12}, KEY: {13}".format(
            self.id, self.shareholder, self.shareholder_type, self.blic_type, self.blic_number, self.ent_type,
            self.con_capital, self.con_capital_date, self.con_paid_in_capital, self.paid_in_capital_date,
            self.share_ratio, self.currency, self.c_id, self.KEY)
...
class Taxcred(Base):
    ...
    def __repr__(self):
        return "id: {0}, c_id: {1}, id_number: {2}, years: {3}, eval_department: {4}, grade: {5}, eval_type: {6}, KEY: {7}".format(
            self.id, self.c_id, self.id_number, self.years, self.eval_department, self.grade, self.eval_type, self.KEY)
...
class TmInfo(Base):
    ...
    def __repr__(self):
        return "id: {0}, c_id: {1}, tm_name: {2}, reg_number: {3}, tm_type: {4}, reg_annc_date: {5}, reg_annc_issue: {6}, property_bgn_date: {7}, property_end_date: {8}, goods_name: {9}, apply_date: {10}, state: {11}, applicant: {12}, apply_process: {13}, pic_url: {14}, KEY: {15}".format(
            self.id, self.c_id, self.tm_name, self.reg_number, self.tm_type, self.reg_annc_date, self.reg_annc_issue,
            self.property_bgn_date, self.property_end_date, self.goods_name, self.apply_date, self.state,
            self.applicant, self.apply_process, self.pic_url, self.KEY)
...
class ToExecute(Base):
    ...
    def __repr__(self):
        return "id: {0}, c_id: {1}, executed: {2}, org_code: {3}, exec_money: {4}, exec_court: {5}, case_create_date: {6}, publish_date: {7}, case_number: {8}, sex: {9}, province: {10}, gist_num: {11}, gist_unit: {12}, duty: {13}, performance: {14}, disrupttype_name: {15}, end_date: {16}, unexectued: {17}, legal_person: {18}, KEY: {19}".format(
            self.id, self.c_id, self.executed, self.org_code, self.exec_money, self.exec_court, self.case_create_date,
            self.publish_date, self.case_number, self.sex, self.province, self.gist_num, self.gist_unit, self.duty,
            self.performance, self.disrupttype_name, self.end_date, self.unexectued, self.legal_person, self.KEY)
...
class WebsiteInfo(Base):
    ...
    def __repr__(self):
        return "id: {0}, c_id: {1}, web_name: {2}, web_site: {3}, examine_date: {4}, liscense: {5}, company_type: {6}, ym: {7}, KEY: {8}".format(
            self.id, self.c_id, self.web_name, self.web_site, self.examine_date, self.liscense, self.company_type,
            self.ym, self.KEY)
...
class YearReport(Base):
    ...
    def __repr__(self):
        return "id: {0}, c_id: {1}, company_name: {2}, report_year: {3}, reg_number: {4}, manage_state: {5}, employee_num: {6}, phone_number: {7}, email: {8}, postal_address: {9}, postcode: {10}, KEY: {11}".format(
            self.id, self.c_id, self.company_name, self.report_year, self.reg_number, self.manage_state,
            self.employee_num, self.phone_number, self.email, self.postal_address, self.postcode, self.KEY)
